...
class NewsSummaryDTO(BaseModel):
    ...
    featured_image: Optional[str] = Field(default=None, description="URL gambar utama")
    author_name: str = Field(description="Nama penulis")
    author_avatar: Optional[str] = Field(default=None, description="Avatar penulis")
    pesantren_name: Optional[str] = Field(default=None, description="Nama pesantren terkait")
    is_featured: bool = Field(default=False, description="Berita unggulan")
    publish_date: Optional[datetime] = Field(default=None, description="Tanggal publikasi")
    views: int = Field(default=0, description="Jumlah views")
